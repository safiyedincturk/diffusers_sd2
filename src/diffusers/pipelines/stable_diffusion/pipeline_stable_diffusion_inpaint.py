...
def prepare_mask_and_masked_image(image, mask):
    ...
    if isinstance(image, torch.Tensor):
        ...
        # print(image[0].shape)
        # PIL.Image.fromarray(np.uint8(np.transpose(image[0], (1, 2, 0)))).save("photos/depth_woimage_cagla/image2mask2.png")
    ...
    # PIL.Image.fromarray(np.uint8((torch.squeeze(image).permute(1, 2, 0)+1)*127.5)).save("photos/depth_woimage_cagla/image2mask.png")
    masked_image = image * (mask < 0.5)
    # print('img shape: ', image.shape)
    # print(torch.squeeze(masked_image).permute(1, 2, 0).shape)
